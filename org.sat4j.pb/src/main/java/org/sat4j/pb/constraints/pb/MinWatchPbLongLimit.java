...
public class MinWatchPbLongLimit extends WatchPbLong {
    ...
    protected void computeWatches() throws ContradictionException {
        ...
        for (int i = 0; i < this.lits.length
                && watchCumulMinusValueIsLessThanDegree(this.coefs[0]); i++) {
    ...
    private void watchMoreForLearntConstraint() {
        ...
        while (watchCumulMinusValueIsLessThanDegree(this.coefs[0]) && free > 0) {
            free = 0;
    ...
    protected void computePropagation(UnitPropagationListener s)
        ...
        while (ind < this.lits.length
                && watchCumulMinusValueIsLessThanDegree(this.coefs[this.watching[ind]])) {
    ...
    public boolean propagate(UnitPropagationListener s, int p) {
        ...
        if (watchCumulMinusValueIsLessThanDegree(this.coefs[pIndice])) {
            ...
            assert Long.MAX_VALUE - this.coefs[pIndice] > maxCoef;
            if (watchCumulMinusValueIsLessThanDegree(this.coefs[pIndice]
                    + maxCoef)) {
                ...
                for (int i = 0; i < this.watchingCount; i++) {
                    assert Long.MAX_VALUE - this.coefs[pIndice] > this.coefs[this.watching[i]];
                    // if (watchCumul - coefs[pIndice] - degree <
                    // coefs[watching[i]]
                    if (watchCumulMinusValueIsLessThanDegree(this.coefs[pIndice]
                            + this.coefs[this.watching[i]])
                            && i != pIndiceWatching
                            && !this.voc
                                    .isSatisfied(this.lits[this.watching[i]])
        ...
        substractToWC(this.coefs[pIndice]);
    ...
    public void remove(UnitPropagationListener upl) {
        ...
        while (ind < this.coefs.length
                && watchCumulMinusValueIsLessThanDegree(this.coefs[ind])) {
    ...
    protected long updateWatched(long mc, int pIndice) {
        ...
        if (this.watchingCount < size()) {
            ...
            if (this.compWatchCumul > 0) {
                assert this.watchCumul == Long.MAX_VALUE;
                compUpWatchCumul = this.compWatchCumul - this.coefs[pIndice];
                if (compUpWatchCumul < 0) {
                    upWatchCumul = Long.MAX_VALUE - compUpWatchCumul;
                    compUpWatchCumul = 0;
                }
            } else {
                upWatchCumul = this.watchCumul - this.coefs[pIndice];
            }
            ...
            for (int ind = 0; ind < this.lits.length; ind++) {
                ...
                if (!this.voc.isFalsified(this.lits[ind]) && !this.watched[ind]) {
                    ...
                    if (Long.MAX_VALUE - this.coefs[ind] >= upWatchCumul) {
                        upWatchCumul = upWatchCumul + this.coefs[ind];
                    } else {
                        compUpWatchCumul = this.coefs[ind]
            ...
            this.compWatchCumul = compUpWatchCumul;
            addToWC(this.coefs[pIndice]);
        }
        return maxCoef;
    }
    ...
    private boolean watchCumulMinusValueIsLessThanDegree(long coef) {
        return this.watchCumul - coef < this.degree
                && this.watchCumul != Long.MAX_VALUE
                || this.compWatchCumul - coef > 0
                || this.watchCumul - coef + this.compWatchCumul < this.degree;
    }

    private void addToWC(long coef) {
        if (Long.MAX_VALUE - coef >= this.watchCumul) {
            this.watchCumul = this.watchCumul + coef;
        } else {
            this.compWatchCumul = coef - (Long.MAX_VALUE - this.watchCumul);
            this.watchCumul = Long.MAX_VALUE;
        }
    }
    ...
    private void substractToWC(long coef) {
        if (this.compWatchCumul == 0) {
            ...
            this.compWatchCumul = this.compWatchCumul - coef;
            if (this.compWatchCumul < 0) {
                this.watchCumul = this.watchCumul - this.compWatchCumul;
                this.compWatchCumul = 0;
            }
        }
    }
